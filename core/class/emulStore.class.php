...
class emulStore extends eqLogic {
	...
	if (!is_object($cmd)) {
	    log::add('emulStore','error',sprintf(__("La commande '%s' est introuvable",__FILE__),'puissance'));
	}
	log::add('emulStore','debug','action: ' . $action);
	if ($action == "ouvre") {
...
class emulStoreCmd extends cmd {
	...
	if ($this->getType() == 'action'){
		...
		log::add('emulStore','error',sprintf(__("La commande '%s' est introuvable",__FILE__),'position'));
		return;
	    }
